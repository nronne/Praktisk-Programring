...
/*
	test = abs(a).pow(2).approx(a.Re*a.Re+a.Im*a.Im) )
	if(
		log(exp(a)).approx(a)
		&&
		log(exp(b)).approx(b)
	...
	if( abs(a).pow(2).approx(a.Re*a.Re+a.Im*a.Im) )
		Write("test abs passed\n");
	else {
		Write("test abs failed.......\n");
		a.print("a=");
		return 1;
		}
	if( (exp(a)/exp(b)).approx(exp(a-b)) )
		Write("test exp(a-b)=exp(a)/exp(b) passed\n");
	else return 1;
	if( (exp(a).pow(b)).approx(exp(a*b)) )
		Write("test exp(a)^b=exp(a*b) passed\n");
	else return 1;

	Write("test log(a/b)=log(a)-log(b)... ");
	if( log(a/b).approx(log(a)-log(b)) )
		Write("... passed\n");
	else {
		Write("... FAILED\n");
		a.print("a=");
